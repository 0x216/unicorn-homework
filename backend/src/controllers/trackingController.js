...
exports.create = async (req, res) => {
    ...
    try {
        const result = await sequelize.transaction(async (t) => {
            const existingTracking = await Tracking.findOne({ where: { userId: user.id } }, { transaction: t });
            if (existingTracking) {
...
exports.get = async (req, res) => {
    ...
    try {
        ...
        if (!savings) {
            return res.status(404).send({ message: 'Savings not found for the tracking record' });
        }
